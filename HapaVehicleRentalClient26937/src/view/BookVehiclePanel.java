...
public class BookVehiclePanel extends JPanel {
    ...
    /** RMI service for vehicle operations */
    ...
    /**
     * Loads available vehicles from RMI service and creates display cards
     * Filters out vehicles in maintenance status to show only bookable vehicles
     */
    private void loadVehicles() {
        try {
            // Connect to RMI VehicleService
            vehicleService = (VehicleService) Naming.lookup("rmi://localhost:3506/VehicleService");
            List<Vehicle> vehicles = vehicleService.getAllVehicles();    // Get all vehicles via RMI
    ...
    private JPanel createVehicleCard(Vehicle v) {
        ...
        // Add booking action - open BookingForm
        btnBook.addActionListener(evt -> {
            if (loggedInUser != null) {
                new BookingForm(loggedInUser, v).setVisible(true);
            } else {
                JOptionPane.showMessageDialog(this, "Please login to book a vehicle", "Login Required", JOptionPane.WARNING_MESSAGE);
            }
        });
        ...
        ImageIcon seatIcon = null;
        
        try {
            fuelIcon = new ImageIcon(new ImageIcon("images/fuel.png").getImage().getScaledInstance(16, 16, Image.SCALE_SMOOTH));
            gearIcon = new ImageIcon(new ImageIcon("images/gear.png").getImage().getScaledInstance(16, 16, Image.SCALE_SMOOTH));
            seatIcon = new ImageIcon(new ImageIcon("images/seat.png").getImage().getScaledInstance(16, 16, Image.SCALE_SMOOTH));
        } catch (Exception e) {
            // Icons not found, will use text only
        }
        
        // Create specification labels with icons
        JLabel lblFuel = new JLabel(v.getFuelType() != null ? v.getFuelType() : "Petrol", fuelIcon, JLabel.LEFT);
        JLabel lblTrans = new JLabel(v.getTransmission() != null ? v.getTransmission() : "Manual", gearIcon, JLabel.LEFT);
        JLabel lblSeats = new JLabel(v.getSeats() > 0 ? String.valueOf(v.getSeats()) : "5", seatIcon, JLabel.LEFT);
        
        // Set icon spacing and font
        if (fuelIcon != null) lblFuel.setIconTextGap(5);
        if (gearIcon != null) lblTrans.setIconTextGap(5);
        if (seatIcon != null) lblSeats.setIconTextGap(5);
